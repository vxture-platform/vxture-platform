...
GRANT UPDATE (account, email, email_verified_at, phone, phone_verified_at, account_changed_at, account_login_disabled, status, source, updated_at, deleted_at, deletion_requested_at, profile_completed_at) ON account.users TO platform_svc;
...
GRANT UPDATE (name, display_name, type, owner_user_id, status, verification_status, verification_type, member_limit, workspace_limit, updated_at, deleted_at) ON tenancy.tenants TO platform_svc;  -- workspace_counter 随 §11 v4 解耦退役(2026-09-05);member_limit/workspace_limit = 防滥用闸的单租户覆盖,运营可改(2026-09-22)

-- tenancy.tenant_profiles  [anchor: tenant_id, created_at]
REVOKE UPDATE ON tenancy.tenant_profiles FROM platform_svc;
GRANT UPDATE (description, industry, scale, website, country_code, address, address2, postal_code, is_billing_recipient, timezone, language, currency, updated_at) ON tenancy.tenant_profiles TO platform_svc;

-- tenancy.tenant_contacts  [anchor: id, created_at]
REVOKE UPDATE ON tenancy.tenant_contacts FROM platform_svc;
GRANT UPDATE (tenant_id, contact_type, name, title, email, phone, user_id, gender, updated_at) ON tenancy.tenant_contacts TO platform_svc;
...
GRANT UPDATE (client_id, client_secret_hash, realm, product_id, client_kind, release_channel, name, display_name, logo_url, redirect_uris, post_logout_redirect_uris, allowed_scopes, access_token_ttl, refresh_token_ttl, pkce_required, token_endpoint_auth_method, slo_participation, back_channel_logout_uri, status, updated_at) ON appoidc.oidc_clients TO platform_svc;
...
-- product.metric_catalog  [anchor: metric_key, created_by, created_at]
REVOKE UPDATE ON product.metric_catalog FROM platform_svc;
GRANT UPDATE (display_name, description, updated_by, updated_at) ON product.metric_catalog TO platform_svc;
...
GRANT UPDATE (plan_id, status, is_locked, trial_cycle_unit, trial_cycle_count, published_at) ON product.plan_versions TO platform_svc;
