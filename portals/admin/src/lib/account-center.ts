 ...
 *
 * 运营者账户自助(改邮箱/通行密钥等)收敛到身份层 accounts 门户的 /account 单点;admin
 * 只出跳转入口——原 admin 内的邮箱自助(OperatorAccountSettings + admin-bff
 * operator-contact 代理)随之退役。accounts 是公开登录门户(accounts.vxture.com,非内部
 * 主机),故 prod 默认直接用它,dev 指本地 3080;可被 NEXT_PUBLIC_ACCOUNTS_URL 覆盖。
 * 同源 vx_sid_op 会话让运营者到那边即已认证(与登录同一中心会话)。
 */
const ACCOUNTS_BASE = (
