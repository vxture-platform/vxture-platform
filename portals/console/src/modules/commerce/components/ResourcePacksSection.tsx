 ...
 * 等于同一件事说两处。
 *
 * ── 标签取名（owner 2026-09-09，来回收敛过三轮）──
 * 标签只答一个问题：**这笔额度哪来的**。额度、周期、到期、来由都在卡片主体里，
 * 标签不必重复。四字：两字（自带 / 已购 / 发放）实测不够清楚，六字会把指标名挤走。
 *
 * `manual_override` 的「平台发放」是**占位**，要点明白：
 * 代码库里**没有任何写入方**创建这种池——它只存在于 DDL 的 CHECK 里。
 * owner 曾想按「卡券兑换」命名，核过之后不成立：卡券（`services/commerce/promotion`）
 * 是订单结算里的抵扣引擎，`parseCreditVoucherEffect` 返回的是 `amountCents`
 * ——**给的是钱，不是额度**，它完全不碰 `quota_pools`。写「卡券兑换」会把租户
 * 引到 /vouchers 去找一条不存在的通路。
 *
 * 所以这里用一个不承诺具体通路的词。**真做出发放通路时，按它实际的形态改名**，
 * 别让这个占位词反过来定义那件事。
 *
