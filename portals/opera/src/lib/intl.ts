...
export type OperaMessages = typeof zhCN;

const CATALOG: Record<Locale, OperaMessages> = {
  "zh-CN": zhCN,
  /* 断言而不是让 TS 结构比对：两本的差异要由守卫报告（哪个键缺了、缺在哪本），
     不该表现为这一行上一坨读不懂的类型错误。守卫见
     scripts/guardrails/check-message-catalogs.mjs。 */
  "en-US": enUS as OperaMessages,
};
 ...
 */
export function operaLocale(headers: Headers): Locale {
  return resolveLocale({ headers });
}

export function operaMessages(locale: Locale): OperaMessages {
