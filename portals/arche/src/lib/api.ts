 ...
 */
export interface ArcheApiErrorBody {
  code?: string;
...
export class ArcheApiError extends Error {
  constructor(
    message: string,
    readonly status: number,
    /** 解析得到的原始错误体；非 JSON 响应时为 null。 */
    readonly body: ArcheApiErrorBody | null = null,
  ) {
    super(message);
    this.name = "ArcheApiError";
  }
...
async function readErrorBody(
  response: Response,
): Promise<ArcheApiErrorBody | null> {
  try {
    const parsed: unknown = await response.json();
    return parsed && typeof parsed === "object"
      ? (parsed as ArcheApiErrorBody)
      : null;
  } catch {
...
export function isStepUpRequiredError(error: unknown): boolean {
  if (!(error instanceof ArcheApiError) || error.status !== 403) return false;
  return (
 ...
 * 调用方随即重试被拒的那个写操作。
 *
 * 码错、过期、或该 operator 根本没绑 MFA，都抛 ArcheApiError。
 */
