 ...
 *
 * 2026-08-31 admin 3204 → 3205：解决方案补删除,`api/admin-bff.ts` 的
 *   `deleteProductSolution` 新增一条兜底错误文案「删除方案失败」——与同文件里既有的
 *   「创建方案失败」「解绑套餐失败」等一整批 API 客户端错误串并列(该模块是普通函数、
 *   没有 t(),错误文案历来硬编码在此),单抽一条会造出半中英,随该文件整体 i18n 时再抽。
 */
const BASELINE = {
  console: 30,
  website: 40,
