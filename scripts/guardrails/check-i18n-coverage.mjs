...
const BASELINE = {
  ...
  console: 0,
  // 2026-09-24 website 39 → 40（+1）：/appcenter 的行业筛选要脱敏（owner「国防 删除」），
  // 而脱敏的判据必须和 DB 里的值逐字相等——`marketing.zh.industries` 存的就是「国防」
  // 这个中文串。它不是界面文案（页面上永远不显示它，正相反：显示它才是 bug），
  // 走 t() 反而会让一个**策略判据**变成可被翻译改写的东西。所以按本守卫头注的逃生口
  // 记名加 1，而不是放宽规则。见 AgentMarketplacePage 的 INDUSTRY_DENYLIST。
  website: 40,
