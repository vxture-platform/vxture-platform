...
/**
 * `actor_console` 恒为 `'arche'`（product_251 X-3）——这个进程只服务一个控制台
 * （治理台 arche），所以是常量而不是入参：让调用方传，迟早有人传错或不传。
 * actor_console 是无约束 varchar(32)（72_support.sql），新增控制台标识无需改 DDL。
 */
...
values
  ('operator', 'arche', $1, $2, $3, $4, $5, $6::jsonb, $7::jsonb, $8, $9)
`;
