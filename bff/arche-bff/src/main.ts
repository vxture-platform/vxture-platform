...
import { ARCHE_BFF_RW_POOL } from "./tokens";
...
async function bootstrap() {
  ...
  if (process.env["BOOT_SMOKE"] === "1") {
    ...
    console.log("[boot-smoke] arche-bff DI graph resolved OK");
    process.exit(0);
  }
  ...
  app.useGlobalFilters(
    new AllExceptionsFilter(app.get<Pool>(ARCHE_BFF_RW_POOL)),
  );
  ...
  setupOpenApi(app, {
    title: "arche-bff",
    description:
      "Governance plane BFF: operator shell for platform identity, RBAC, audit, compliance and config.",
    version: process.env["npm_package_version"] ?? "0.0.0",
  });

  await app.listen(Number(process.env.ARCHE_BFF_PORT ?? 3051));
}
