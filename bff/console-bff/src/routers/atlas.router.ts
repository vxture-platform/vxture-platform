 ...
 * ── 2026-09-09:当前无消费方 ──
 * console 的 /atlas 页面已删(owner 裁定:模型服务 / 技能工具不需要 console 提供
 * 页面,菜单项直接跳文档站)。本 router 三条 GET 因此**没有任何调用方**。
 *
 * 保留而不是一并退役,是因为 owner 的原话是「数据**先**不要」——「先」意味着
 * 这三块(可用模型 / 产品权益 / 配额与用量)可能换个地方回来,而重新接通 S2S
 * 换票那条链比留着它贵得多。留着的代价只是一段没人调的代码;它仍受
 * tenant.model.read 门控,不是敞口。
 *
 * **要么接回消费方,要么整体删掉**——长期挂着没有调用方的路由会烂。
 */
