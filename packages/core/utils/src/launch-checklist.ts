 ...
 *
 * ── 退役的两项 ──
 *   `acceptance`   2026-11-03 退出检查单。**那条检查照跑**（opera 的复验页仍然实测
 *                  五段痕迹），退的是它在这张表上的席位：检查单答的是「还差哪几件
 *                  才能上线」，而它不办也能上线（发布门已改读 certification_runs）。
 *                  结果改由**运行健康**呈现——三态 healthy / degraded / unknown，
 *                  没有「未通过」这个说法。对一个刚上线、还没有客户的产品，链路当然
 *                  是空的；画成红色等于把「没人用」说成了「坏了」。
 *
