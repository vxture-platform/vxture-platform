 ...
 * @category Utils
 *
 * 2026-09-21 从 `portals/console/src/lib/principal-no.ts` 上提到 @shared。
 * 原因是 owner 实看 admin：「所有的租户账号显示中，缺失了 T- 前缀」「同样检查
 * U-、T-、W- 前缀，所有显示页面」——admin 有 62 处在裸显示这三种码。
 * 各门户各抄一份正是本文件当初要消灭的那件事（头注里那三套写法），所以放这里，
 * console 侧改成薄再导出，调用点不动。
 *
