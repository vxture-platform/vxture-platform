...
wire 值域即上表六个 slug（`GET orders` 的 `orderStatus` 契约 + i18n key 同源），替换现有 pending/confirmed/closed 三值。

- **修订（2026-09-25）——六态扩为十态，派生源换成订单实体**：上表那套「invoice + sub 有序谓词」已退役，现在一律读 `billing.orders.status`（product_330 P1-b2）。新增四值：`refunded` 从 `cancelled` 里**分出**（此前两者同一支，于是一张退过款的单显示「已取消 · 未付款」——而它付过钱，这是唯一一句关于钱的假话）；`partially_paid` 由账单 `bill_status='partial'` 派生，**只在 `pending_payment` 上判**（确认收款不足额时订单实体不动、钱只落账单）——`pending_verify` + partial 刻意保持「待核对」：`markDeclaredTx` 只接 `pending_payment`，客户无法二次申报，这张单等的是运营再确认剩余款；改写它既会给客户一个必定 409 的付款按钮，也会与 owner 2026-09-08 的运营待办告警裁定打架（`check-ops-todo-alerts` 拦住了这一版的第一稿）；`refunding` 由 `billing.refunds` 在途单派生（待审，或审过未落定；被驳回与执行失败都不算在途）；`partially_refunded` 判**金额**不判 `refund_type`（退成功合计 < 实付合计），折算退上线前不可达，值域先留以免上线当天再改一次契约。admin 侧同步：`mapEntityOrderStatus` 把 `refunded` 从 `closed` 拆出（`pending_verify` 那一支保持恒等，理由同上），`mapPaymentStatus` 在账单 partial 时先回 `partial`（只看腿会把「收了一半」说成已支付）。完整状态机图、逐条转换闸门与两轴组合见 owner 定稿文档《订单与服务状态机》（2026-09-25）。**"invoice 未清"全文统一为 `bill_status IN ('unpaid','partial')`**——PR0 同步修订 320 O1 判定谓词（原文只写 `unpaid`，查不到 partial 存量单）并点名同步改动点：subscribe-context `pendingOrder` 查询、下单重复挂单 409 守卫。
