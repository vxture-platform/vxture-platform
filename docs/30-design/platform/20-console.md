...
>
> 2026-09-07（owner）：智能体页两张卡的卡底**照官网 `ProductCatalogCard`**（`/products` 与 `/appcenter` 共用，2026-09-03 定稿），不另造：**卡底一行**，左边 `v 1.2.3 at 2026/9/12`（模板与官网同一份，日期按 locale 数字格式，zh 不补零），右边动作——「产品介绍」走文字链（次要去处），主按钮在最右。版本行与动作同一行，不另起一行占高度。样式仍走 DS 语义类：官网那张用的是 website 自己的 `vx-*` 营销层，console 禁自造样式层，所以是**照骨架不照类名**。
>
