...
>
> 2026-09-06（批 9，owner 裁定）：「成员与权限」整组撤销，租户侧最终只留**账号信息 / 租户信息 / 成员管理**三个板块。邀请记录成为成员管理页里的一段；角色管理成为它的二级页 `/members/roles`——角色是平台整体定义、租户不可自定义，**只提供查看**。二级页不进菜单树（`/tenant/verification` 同例），旧地址 `/roles`、`/invitations` 只保留跳转。
