...
- **退役列**（P1 只停写、P2 删）：`order_no`、`activation_method='offline_purchase'` 语义、`payment_ttl_minutes`、账单备注里的 intent JSON。
