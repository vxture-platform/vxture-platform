...
**撤掉了原计划里的一项：「延长配额 `period_anchor`」。** 先验它会不会动——不动。`period_anchor` 全仓**只有写入方**（建池时、某条订单路径重置为 `now()`），配额重置的真实判据是 `pg-consume.repository.ts` 里 `needsReset()` 与 `current_period_start = date_trunc(...)`，**按日历对齐**，不读锚点。改它是个看起来生效的空操作；而真要把 `current_period_start` 往后推，效果恰好相反——客户会**更晚**拿到下一期免费额度。

配额这一侧的公道由顺延本身给：多出来的那些天落在某个日历月里，那个月有它自己的额度。（顺带记录一处既有不一致：`50_metering.sql` §3 注释写着「按订阅锚定推进，非日历对齐」，与代码相反。不在本批修，已报 owner。）
...
**未做**：owner 要的「详情弹窗 + 倒计时」。倒计时缺数据源——库里只有 `paused_at` 与最长暂停期 60 天，拿 60 天倒计时等于给客户一个平台没承诺过的时间。两条路待定：给 episode 加一个**选填**的「预计恢复时间」（运营暂停时填，有就倒计时、没有就显示已暂停天数），或只计时不倒计时。
