...
| 基础设施:不做用户订阅,不进产品目录(v1.4,owner 2026-09-23) | atlas / runos                                                                             | 自 v1.4 起**连目录行也不在**(`product.products` 里的行**彻底删除**,不是软删;OIDC 客户端降为 `client_kind='platform'`,与 opera/admin/console 同类);atlas 五档骨架、以及把它们搭售进别人套餐的那些 bundled 组件,一并删除。授权主体是产品——atlas 模型授权(`product-endpoint-grants`,产品轴 + `endpointCode`)、runos 能力授权(`capability-grants`,`subjectType=product`),均在 opera 配,**这一半不变**。配额路径**已定**(见 §7 v1.4):v1.3 写的 bundled 组件路径退役,实际在跑的是 L0 平台计量池(`quota_pools.product_id IS NULL` 的工作区级池) |
...
v1.3 已经把它们移出订阅行,但**目录行仍在**——于是它们继续出现在产品目录、产品监控、层级下拉里,只是"不发套餐"。这一版做完那半步:`product.products` 里的 atlas / runos 行**彻底删除**,五个 atlas 套餐骨架、`seat.max` 登记与全部外键引用一并删除,两个 OIDC 客户端由 `client_kind='product'` 降为 `platform`(与 ruyin 同型,与 opera/admin/console 同类)。
...
**「活着的产品不能是 L1」焊进数据层**:`chk_products_live_layer_not_l1`,写侧 opera 的层级下拉同步收到 L2/L3(`PRODUCT_LAYER_CHOICES`),`lint:catalog-domains` 锁这一对。只删两行等于只处理了今天这两个;规则要有判据,否则明天在 opera 里登记一个 L1 产品照样成功。**L1 仍留在 `PRODUCT_LAYERS` 值域里**——它是平台分层的架构词汇,说的是"这一层存在",而新增的那条 CHECK 说的是"没有**目录商品**站在这一层",两句话不是一回事。
