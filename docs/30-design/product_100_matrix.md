...
> 版本:**v1.3** · 日期:2026-08-31(v1.2 = 2026-08-10,v1.1 = 2026-07-28,v1.0 定稿 2026-07-06) · 状态:**已定稿**(产品名称为终版,owner 拍板,稳定使用,除非再次修订)
> v1.3 修订:**首批对接角色定稿 + vxtpl 补登**(owner 决定 2026-08-30:atlas/runos 为基础设施、不做用户订阅、授权面向产品;karda/arda 走订阅;vxtpl 为第一个智能体·模板),见文末修订记录;v1.2 修订:**runos 行按商业能力面改写**(owner 决定 2026-08-07,runos `ADR-003`;办理 = issue [#216](https://github.com/vxture/vxture-platform/issues/216)),见文末修订记录;v1.1 修订:**ontos L1→L2 重定位**(owner 拍板 2026-07-28);其余不变。
...
| `ontos`      | Ontos    | **L2**(v1.1) | ontos.vxture.com                                            | 是                                                                 | 是                                          | Schema 资产授权模式待产品定义         | —                               | 待定义                                                                            | **未登记**(2026-08-30 起 seed 不再预建无目录行的客户端,定义完成后经 opera 产品目录登记,见 `opera/40-product-registry.md` §5);产品定义空白;**2026-07-28 重定位 L2**(面向用户高频构建操作,非纯 API 能力管道,见修订记录) |
...
| `raven`      | Raven    | L3           | raven.vxture.com                                            | 是                                                                 | 是                                          | 消费方 + 其 A 级资产可被 grant        | —                               | **是**                                                                            | **未登记**(同 ontos:定义完成后经产品目录登记);行业定位待产品定义                                                                                                                                                      |
...
| `vxtpl`      | Vxtpl    | L3(模板)     | —(仓模板参考实现;域名按 `{code}.vxture.com` 规则随部署分配) | 是(client `vxtpl` 已 seed)                                         | 是(首批以订阅产品形态验证,free 套餐)        | 消费方                                | —                               | **是**(product_240 §2.4 三契约 schema)                                            | **首批对接产品**(owner 2026-08-30 定稿:第一个智能体·模板,目标 = 验证整条接入线并成为后续智能体样板;仓 `vxture-vxtpl` v0.2.0);此前只登记在 seed 与 product_240 §7,未入本表,2026-08-31 补登                             |
...
| 机制                                                      | 参与产品                                                                                  | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| workspace × product 订阅(state × tier)                    | ontos / arda / karda / terra / raven / anlan / forge / xuanzhen / vxtpl                   | ADR-11 权益引擎;每产品 5 档(free→enterprise)。**atlas / runos 自 v1.3 起移出本行**(见下一行)                                                                                                                                                                                                                                                                                                                                                |
| 基础设施:不做用户订阅,不进产品目录(v1.4,owner 2026-09-23) | atlas / runos                                                                             | 自 v1.4 起**连目录行也不在**(`product.products` 软删,OIDC 客户端降为 `client_kind='platform'`,与 opera/admin/console 同类);atlas 五档骨架随之软删。授权主体是产品——atlas 模型授权(`product-endpoint-grants`,产品轴 + `endpointCode`)、runos 能力授权(`capability-grants`,`subjectType=product`),均在 opera 配,**这一半不变**。配额路径见 §7 v1.4「待 owner 定的一项」:v1.3 写的 bundled 组件路径随目录行一并失效,实际在跑的是 L0 平台计量池 |
...
### v1.4(2026-09-23)— atlas / runos 退出产品目录(owner 决定 2026-09-23)

**依据**:owner「L0、L1 层级的产品都是平台的基础环境,不应出现在平台的产品中」「基础平台的 OIDC 可以直接在部署的服务器环境中,这**原则上等同 opera、admin、console**,根本不是面向客户的订阅产品」。

v1.3 已经把它们移出订阅行,但**目录行仍在**——于是它们继续出现在产品目录、产品监控、层级下拉里,只是"不发套餐"。这一版做完那半步:`product.products` 里的 atlas / runos 行软删,五个 atlas 套餐骨架软删,`seat.max` 登记清除,两个 OIDC 客户端由 `client_kind='product'` 降为 `platform`(与 ruyin 同型,与 opera/admin/console 同类)。

**换票受众另立落点**:此前 atlas / runos 能被 `aud=` 指向,靠的正是那两行产品行(`resolveTargetProductCode` 查 `product.products`)。现在改由「active 产品 **或** 平台级基础设施客户端」两支解析(`auth-bff` 的 `PLATFORM_LEVEL_S2S_TARGETS`),同时给产品那一支补上此前缺失的 `deleted_at is null`——判据从**商品目录**搬回**客户端表**,karda/arda 的 C1 出站与 opera/admin 自己的管理面调用(`aud="atlas"`)都走这条。

**「活着的产品不能是 L1」焊进数据层**:`chk_products_live_layer_not_l1`(软删行不受约束,历史事实保留),写侧 opera 的层级下拉同步收到 L2/L3(`PRODUCT_LAYER_CHOICES`),`lint:catalog-domains` 锁这一对。只软删两行等于只处理了今天这两个;规则要有判据,否则明天在 opera 里登记一个 L1 产品照样成功。

**本次已改**:§5「基础设施」行的配额路径改写(见下);本节新增 v1.4。§2 的 atlas / runos 两行保留——它们仍然是平台的 L1 组成,只是不再是目录商品;「订阅(entitlement)」列的 `是` 自 v1.3 起已由 §5 覆盖。

**待 owner 定的一项**:v1.3 写的配额路径是「经订阅产品套餐里的 **bundled 组件**进池」,而 bundled 组件指向 `product_id`——atlas / runos 不是产品之后这条路不存在了(库里从来没有一行这样的组件,迁移的前置检查也会拦住它)。实际在跑的是 **L0 平台计量池**(`product.platform_metrics` 的 `ai.credit` / `compute.*`,`quota_pools.product_id IS NULL` 的工作区级池),与 owner「配额只给有成本的」一致。两者取其一需要拍板,本文先如实记下分歧。

**随之而来的**:opera 的「服务监控」以 `product.products` 为主表,atlas / runos 因此从该页消失——平台基础环境的存活探测需要另一个落点,未在本批处理。

### v1.3(2026-08-31)— 首批对接角色定稿 + vxtpl 补登(owner 决定 2026-08-30)

**依据**:owner 定稿首批对接产品 = atlas / runos / karda / arda / vxtpl,三种角色:**基础设施**(atlas、runos——不做用户订阅,授权面向产品)、**通用业务订阅**(karda、arda)、**智能体模板**(vxtpl——第一个智能体,目标是把接入线跑通并成为后续智能体样板)。ruyin 不在首批。

**本次已改**:§2 补登 `vxtpl` 行(此前只在 seed 与 product_240 §7 存在,矩阵未列);§5 把 atlas / runos 从「workspace × product 订阅」行移出,新增「基础设施」行写明授权与配额路径(产品轴授权 + bundled 组件进池)。§1 分层图、§3 定位卡不改——L1「API-first、零端用户 UI」的不变量与本次一致,只是把"用户订不订"说清楚了。

**随之而来的实施项**(不在本文代做):admin 套餐版本需要能挂 bundled 组件(此前只有 primary 入口);opera 目录页对 atlas / runos 关掉「客户端可见」与「可单独订阅」;执行台账见 workplans 产品对接首批计划。
