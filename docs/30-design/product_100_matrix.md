...
| 基础设施:不做用户订阅,授权面向产品(v1.3,owner 2026-08-30) | atlas / runos                                                                             | 不进客户目录、不发用户套餐(atlas 五档骨架保持草稿不发布);授权主体是产品——atlas 模型路由授权(`product-endpoint-grants`,产品轴 + `endpointCode`)、runos 能力授权(`capability-grants`,`subjectType=product`),均在 opera 配;工作区配额经订阅产品套餐里的 **bundled 组件**(product_220 §2)进池,不来自 atlas/runos 自己的套餐 |
