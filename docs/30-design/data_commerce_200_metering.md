...
| `auto_renew`                | boolean       | NOT NULL DEFAULT **false**（owner 2026-09-03）                                                                     | 到期是否自动续期。**默认关**，客户在订单确认页显式开启（`billing.orders.auto_renew` 履约时写入），事后可在「我的订阅」改；**trial 恒 false**；perpetual 无意义 |
...
| ~~`order_no`~~              | —             | **已删（product_330 P2-e，2026-09-06）**                                                                           | 订阅可视码 = `current_order_id → billing.orders.order_no`；旧列与 `payment_ttl_minutes` 一并退役                                                               |
