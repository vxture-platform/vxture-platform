...
> 上游规范（唯一权威）：[产品接入通则](https://claude.ai/code/artifact/20e37039-ea59-4c5d-9fab-ba3ca3ad54f0) · 「API 形状 · MUST 条款全集」
> 旧条款号对照与沿革：[`product_251`（已归档）](../../../30-design/product_251_management-api-conventions.md)
> 实测背景：[`30-l1-consistency-audit.md`](../../../70-workplan/30-l1-consistency-audit.md) §C6

本文是 **platform 自己这一面怎么落地**的仓内口径——写新端点时照这里，不必每次回去翻规范。
**它不复述条款**：条款在接入通则，这里只写「本仓的哪个端点用哪个动词、为什么」。
符合性也不在这里自陈了，见 §6。
...
**这一节曾是一张手写的符合性表，现在删了。**规范自己的 D-3 早就写着：「写死在文档里的符合性是
快照，会烂。**能自动校验的条款就不要靠人记**。」——2026-08-29 复核时，这张表连同另外三处记载
互相矛盾，而人工重数还错了两次（范围划错、把注释当代码）。

| 判据                                                      | 谁来判                               |
| --------------------------------------------------------- | ------------------------------------ |
| 裸 Nest 异常 · 错误码形状 · 审计字段名 · `state`/`status` | `pnpm lint:api-conventions`          |
| 拒绝词表与 `retryable` 的计数 · 封套字段声明 · 与快照比对 | `pnpm lint:conformance`              |
| **B-1 动词语义**（`PUT` 是不是真全量替换）                | **判不了，靠评审**——照 §1 的逐端点表 |
| **B-2 写入结果可分辨**                                    | **判不了，靠评审**                   |

两个守卫都在 CI 里。**判不了的不假装能判**——那两条要读懂业务，脚本给不出结论，
所以它们留在 §1 的表里，由改端点的人自己对。

**本面的适用范围声明**（这部分是判据，不是状态，所以留在文档里）：

- **X-2 归因**：platform 自身管理面不产生 agent 调用，本条对本面不适用。
- **A-3 分页**：本面无无界流水；审计读端点走钳制 `limit`，基数受留存策略约束。
- **B-2**：本面不存在「创建命中已存在对象」的模式。
- **B-4**：本面无 `DELETE`——退役是状态迁移不是删行（§1）。
- **G 组**：本面不面向 agent。
- **X-4**：本仓不拥有 `grants` / `endpoints` 两个撞名词；上游改名后适配层跟着改。
- **D-1**：登记了一条**有失效条件**的豁免，见 §5.1——**出现第二个消费方即失效**。
  这条已同步进接入通则的豁免登记，加消费方的那个人有义务回来改 §5.1。

**新增写端点时要回来改的是 §1 的表**，不是这一节。
