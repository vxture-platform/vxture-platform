...
| `DELETE /api/products/solutions/:code`             | 删除（软删 `deleted_at`）——与退役并列的另一出口，给误建方案用（owner 2026-08-31）            | 判据「无客户足迹即可删」：绑定套餐上有 `active`/`trialing` 订阅 → 409（只能退役）；放行时解绑 `solution_products` / `solution_plans`（释放 plan 供再绑）；不存在 → 404 | `product.solution.delete`           |
...
| 页                           | 读                                                         | 写                                                                                                                                    |
| ---------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `/product-solutions`         | 列表 + 四个筛选 + 真实计数卡                               | 「新建方案」DialogForm（建草稿后跳详情）；行菜单：启用 / 停用 /（确认框）退役 /（确认框）删除                                         |
...
- 多币种 MRR。
- ~~方案的软删接口~~ → 已实现（2026-08-31，`DELETE /api/products/solutions/:code`，见 §7）：退役与删除并列——退役=可见终态，删除=误建方案的出口（软删 `deleted_at` + 解绑，判据无客户足迹）。门户列表行菜单「删除」（`confirm` 二次确认 + step-up）。
