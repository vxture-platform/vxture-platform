...
>
> **2026-09-14：第 2–5、8 步收进一张页。** 此前它们散在目录页「登记产品」弹窗、「接入凭据」页「注册客户端」、详情页的 webhook / 回调地址 / 授权页展示几个弹窗与独立的「产品上线」页。现在是**产品页一张页 + 「密钥管理」一个面板**：新建走 `/product/catalog/new`，产品、边缘与回调、登录客户端一次「创建草稿 / 保存设置」写入（同一事务）；签发新客户端或改回调 / 登出回跳 / scopes / PKCE 时，保存要过一次二次验证。「接入凭据」页只剩全部产品客户端的只读总览。
...
| 2   | 登记产品行                   | opera → 产品目录 → 接入产品（一张页）                 | 否         |
| 3   | 发 OIDC 客户端               | 同一张页 → 登录接入                                   | 否         |
| 4   | 登记 webhook 与边缘上游      | 同一张页 → 边缘路由与回调；签名密钥在「密钥管理」面板 | 否         |
| 5   | 登记计量指标（要计量才需要） | 同一张页 → 计量指标（创建草稿后）                     | 否         |
| 6   | 边缘生效                     | 自动（下次 deploy 的边缘同步）                        | 借一次发版 |
| 7   | 配套餐与定价                 | admin → 服务套餐                                      | 否         |
| 8   | 过上架检查项                 | 同一张页 → 接入检查抽屉                               | 否         |
...
新产品落**草稿**状态。上线走页面底部「接入检查」抽屉里的「确认上线」——它先重跑一遍复验，全通过且检查单必填项齐才转为已上线。
...
产品页 → **登录接入** 板块 → 「添加正式渠道」（灰度 / 金丝雀同理，一个产品每个渠道一个客户端）。它与产品行**同一次保存**写入，新建页上也能直接添加。签发新客户端、改登录回调 / 登出回跳 / scopes / PKCE 时，保存要过一次二次验证（TOTP）；只改展示名与 logo 不打扰。

| 字段                      | 说明                                                                                         |
| ------------------------- | -------------------------------------------------------------------------------------------- |
| `clientId`                | 惯例等于产品码                                                                               |
| `productId`               | 自动挂在本产品上，不用选                                                                     |
...
**client secret 只在保存（签发）与轮换两个动作里明文返回一次**，别处（含列表）永不下发。
保存后弹出的交接清单里就是它，拿到后立即交给产品侧——控制台零持有明文。轮换在页面底部的「**密钥管理**」面板。
...
产品页 → **边缘路由与回调** 板块（域名 / 上游 / 回调地址 / 主页）；**签名密钥在页面底部的「密钥管理」面板里单独登记**（挂二次验证，明文只在保存后显示一次，可以用「生成」给一把 32 位字母数字）。各项都可以先留空（常见的是先拿到回调地址、密钥还没签发），检查项会区分「没有登记行」「配了一半」「配齐」三态。回调路径固定为 `/api/webhooks/vxture`，填别的路径会被拒。

| 字段                | 取值 / 约束                                                      | 填错时                                   |
| ------------------- | ---------------------------------------------------------------- | ---------------------------------------- |
| `homeUrl`           | 产品主页（展示用，不参与投递）。必须是 http/https 绝对地址       | `VALIDATION_INVALID_URL`                 |
| `webhookUrl`        | 平台→产品的推送目标。同上                                        | `VALIDATION_INVALID_URL`                 |
| **`webhookSecret`** | 签名密钥**原文**，只进不出。**至少 16 位**。在「密钥管理」面板填 | `VALIDATION_TOO_SHORT`                   |
...
| `webhookSecretRef`  | **旧路径，新产品不填**。面板里只对已有引用的存量产品显示。见下   | `VALIDATION_TOO_LONG`（>128）            |
...
产品页 → **计量指标** 板块（新建时先「创建草稿」，之后在这里配）。**指标键是跨仓契约**：产品按这个键上报用量（C3 consume），
...
- **上架检查项**：产品页 → 底部「接入检查」抽屉（复验、检查单、交给对方的清单、确认上线）。`verification_policy` 与 `pricing_set`
