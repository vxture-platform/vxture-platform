...
# C11 · P2 两条：都不是「等数据」，都是等人去做（2026-08-24）

计划把这两条归为「需要真实数据才能验」。**两条都验了，而且都不是数据问题。**

## P2.4 · `taskId` 跨产品串联

### 计划说的与实际不同

计划写「本机 dev 数据里两边的 taskId 没有交集（各是各的冒烟批次）」。查 runos 注册表：

```
github.repo-reader              → https://api.githubcopilot.com/mcp/
markitdown.document-to-markdown → http://markitdown:3001/mcp/
runos.code-sandbox              → http://executor:3210/mcp
runos.td019-live-check          → http://executor:3210/mcp
```

**当前能力目录里没有任何一条端点打到 atlas。** 所以不是「碰巧没重叠」，是**没有任何路径能
产生重叠**。这个区别要紧：前者等一批数据，后者等一个会调模型的能力被注册。

### 而门户这边，接缝本来就是断的

三处都半接着，任何一处都足以让这条路走不通：

|              | 状态                                                                                |
| ------------ | ----------------------------------------------------------------------------------- |
| atlas 表     | **没有 taskId 列**——它只出现在「复制整行」的文本里                                  |
| runos 调用流 | **不能按 taskId 过滤**——关键词打在 `capabilityId` 上，只有「任务反馈」那条按 taskId |
| 两张表之间   | 无共享状态                                                                          |

atlas 那侧的注释写着「把值贴到两边，就能看到同一次任务在模型面与能力面各发生了什么」
——**贴不进去**。这条接缝**即使有交集数据也走不通**，而这一点在把它归为「等数据」时
没有被发现。

### 做法：一个 taskId 驱动两张表

`taskId` 提升到页面级：atlas 表补可点的任务列、runos 调用流接上 taskId 过滤、
两边的值都可以直接点。串联态用横幅明说，带「取消串联」。

**最要紧的是空态。** 串联下的空**是一个答案，不是一次失败**——按上面那条发现，
「模型面有、能力面没有」正是当前目录下的**预期结果**。所以两侧各写各的：

> 任务 X 在能力面没有记录。若它在上方 Atlas 请求日志里有行，说明这次任务只碰了模型面
> ——这在当前能力目录下是正常的。

写成通用的「没有匹配的记录」，会让人去查权限和网络。

实跑：点 atlas 一行的 taskId → 横幅出现、atlas 侧 50+ 收窄到 8 行、runos 侧给出上面那句。

## P2.5 · atlas `blocked` —— 演练做了，抓出两个上游缺陷

计划写「等有预发环境或一次计划内演练」。本地就有 atlas 和它自己的库，这就是演练：
停 `vx-atlas-postgres-db-dev`，观察，恢复。

### 先证实了那个陷阱本身

```
/healthz  HTTP 200
/readyz   HTTP 200   ← 一个彻底不可用的 atlas，两个端点都回 200
          status: "blocked"
```

服务状态页确实把「存活=健康」与「就绪=不可达」分开显示了，翻译没错。**但就绪那格写的
是「探测超时」而不是 blocked**，于是往下查发现两件事。

### 缺陷一：名叫 database 的检查，是唯一不查数据库的

停库时 `/readyz` 报 `database=pass`，而每一个真的发查询的检查全 fail。原因：

```ts
checkDatabaseConnectivity(): Promise<void> {
  return prisma.$connect();     // 已连接时是空操作，不发任何查询
}
```

**这比没有检查更坏。** 从那张表上读出来的结论是「库没事，是注册表代码坏了」——真相的
反面，会把人在故障中指向错误的系统。改成真实往返（`SELECT 1`）。

### 缺陷二：blocked 时 `/readyz` 要二十秒，于是没人看得见它

实测 **19.8s 与 23.0s**——每个检查都坐在一个永远拿不到的连接上。而控制台的探测超时是
4 秒，所以它每次都记「不可达 / 探测超时」。

> 那个被仔细计算、并在消费方用单测钉过的 `blocked` body，**一次都没有在实践中被观察到**。

一个就绪端点，**在一切正常时最快、恰恰在答案最要紧时最慢**，方向是反的。加每检查
2 秒截止：超时的检查算 **fail**（不是 unknown），所以汇总仍是 `blocked`，`checks` 仍
点名哪个依赖哑了——调用方拿到的比一个探测超时**多**，不是少。

### 修复后的实测对比

|                      | 修复前         | 修复后                                          |
| -------------------- | -------------- | ----------------------------------------------- |
| `/readyz` 耗时       | 19.8s / 23.0s  | **2.005s**                                      |
| `database`           | `pass`（撒谎） | `fail` + 「check did not answer within 2000ms」 |
| BFF 看到的 readiness | `unreachable`  | **`fail`**（走的正是 `blocked` 分支）           |
| BFF 的 error         | `探测超时`     | `null` —— 不是故障，是答案                      |
| BFF 的 checks        | `null`         | 六项逐个点名                                    |

`readinessFromBody("blocked", 200)` 这条分支，**至此第一次在真实 atlas 上被走到**。

## 记一条方法上的事

这两条被归为「需要真实数据」，而实际上：一条是**接缝根本没接上**（与数据无关），
另一条**本地十分钟就能演练**（不需要预发环境）。
