...
> **状态**（2026-08-29）：**立项评估；B0 度量已做（当天），其余未开工。** 结论在 §6，工作量在 §5。
...
| 批                            | 内容                                                                                                                                                                                                                                                                                                                                                                                                           | 文件                                                                                              | 估算                          |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------- |
| **B0 度量**（先做，独立可发） | 两个 guard 记 `warn` 日志：`captcha_verification_failed surface= reason= ip= detail=`（**不记 token**；`reason=missing-token` 就是客户端 12 秒降级/脚本失败那条）；`AuthTurnstile.onError` 的六位码与 `timeout` 透到 widget 下方文案。~~Prometheus 计数~~——**平台任何 BFF 都没有 metrics 栈**，为半天的活不引一套；日志前缀稳定，`docker logs vx-platform-auth-bff \| grep captcha_verification_failed` 就能数 | `operator-login-guard` · `tenant-login-guard` · `OidcLoginForm` · `BindPhonePanel`                | **0.5 天**（2026-08-29 已做） |
...
1. **先做 B0**——半天，独立 PR，独立发版。它让 §2 的盲点消失：一周之后按 `captcha_verification_failed` 日志的 `surface`/`reason` 分布能直接回答「国内用户到底有没有在这上面掉」。
2. **B1–B3 可以并行准备**，但**切换（B4）等 B0 的数字**：若一周内租户面的失败里 `reason=missing-token`（客户端 12 秒降级/脚本失败）与 `network-error` 占比可忽略，就不切租户面，只把运营面按 owner 裁定处理；若不可忽略，按 B4 顺序切。
